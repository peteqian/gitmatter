...
- **Terms copy:** links to `/terms` and `/privacy` (open in new tab).
...
- Session storage key: `gitmatter:mfa-verified-at`.
...
2. `startEnrollment()`:
   - `supabase.auth.mfa.enroll({ factorType: "totp", friendlyName: "gitmatter" })`.
   - On duplicate friendly name error, retries with `gitmatter ${Date.now()}`.
...
## 17. API — Auth/Account functions

**File:** `frontend/src/app/lib/api.ts` (auth/account relevant excerpts)
