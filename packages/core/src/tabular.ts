...
import { DEFAULT_MODEL, completeClaudeText } from "./llm/claude.js";
...
export async function queryCell(params: {
  ...
  const prompt = `${params.columnPrompt}${formatSuffix(params.format)} If not found, state "Not Found". Put all reasoning in the "reasoning" field only.`;
