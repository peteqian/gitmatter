...
function Button({
  ...
  const button = (
    <ButtonPrimitive
      ...
      // Icon buttons carry no text; the tooltip doubles as the accessible name.
      aria-label={ariaLabel ?? tooltip}
      {...props}
    />
  );

  // Instant (delay-0) tooltip instead of the native `title` attribute, which the
  // browser opens after a ~500ms delay that can't be configured.
